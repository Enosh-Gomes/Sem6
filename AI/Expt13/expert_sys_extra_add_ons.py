...
def load_diseases():
    ...
    json_path = os.path.join(current_dir, "disease_symptoms1.json")
    with open(json_path, "r") as file:
        knowledge_base = json.load(file)
    return knowledge_base

# FUNCTION : Filtering / Discriminating Questions
def get_discriminating_symptoms(candidates, diseases):
    ...
    for symptom in set(s for d in candidates for s in diseases[d].keys()):
        have_symptom = sum( 1 for d in candidates if symptom in diseases[d])
        dont_have = len(candidates) - have_symptom

        # Balanced split = better question
        if have_symptom > 0 and dont_have > 0:
            balance = (min(have_symptom, dont_have) / len(candidates))

            # Average weight of symptom
            avg_weight = sum(diseases[d][symptom] for d in candidates if symptom in diseases[d]) / have_symptom
            symptom_scores[symptom] = balance * avg_weight

        elif have_symptom > 0:
            avg_weight = sum(diseases[d][symptom] for d in candidates if symptom in diseases[d]) / have_symptom
            symptom_scores[symptom] = 0.01 * avg_weight
...
def dynamic_diagnosis():
        ...
        if answer in ['yes', 'y']:
            for disease in knowledge_base:
                if question in knowledge_base[disease]:
                    weight = knowledge_base[disease][question]
    ...
    if highest_score <= 0:
        ...
        print(f"You are most likely having: {best_disease}")
